...
static ZWLRUManager *manager = nil;

static NSMutableArray *operationQueue = nil;

static NSString *const ZWLRUManagerName = @"ZWLRUManagerName";


@implementation ZWLRUManager

+ (ZWLRUManager *)shareManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[ZWLRUManager alloc] init];
        if ([[NSUserDefaults standardUserDefaults] objectForKey:ZWLRUManagerName]) {
            operationQueue = [NSMutableArray arrayWithArray:(NSArray *)[[NSUserDefaults standardUserDefaults] objectForKey:ZWLRUManagerName]];
        }else {
...
- (void)addFileNode:(NSString *)filename {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[operationQueue copy] forKey:ZWLRUManagerName];
